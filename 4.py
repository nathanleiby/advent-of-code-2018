    ...
    for r in records:
        ...
        for idx, val in enumerate(minutes):
            if val == "#":
                cnt[idx] += 1
    print(cnt)
    # 1 item, return key (the "minute", not the count)
...
with open("./4-input", "r") as f:
    ...
    max_sleeping_guard = max_sleep_time(guard_to_time)
    print(max_sleeping_guard)
    filtered_rs = list(filter(lambda r: r[1] == max_sleeping_guard, rs))
    print(records_to_str(filtered_rs))
    # print("".join(lines))
    sleepiest_min = find_sleepiest_minute(filtered_rs)
    print("RESULT => ", max_sleeping_guard * sleepiest_min)
