...
# IDEA:
# 1. write all the claims
# 2. go through all claims again, and check if unbroken

def is_claim_intact(df, claim):
    num, x, y, height, width = claim

    intact_claim = pd.DataFrame(index=range(y, y+height), columns=range(x, x+width))
    intact_claim = intact_claim.fillna(True)

    actual_claim = df.loc[range(y, y+height), range(x, x+width)] == 1

    return actual_claim.equals( intact_claim)

assert(is_claim_intact(df_small, (3,5,5,2,2) ))

with open("./3-input", "r") as f:
    lines = f.readlines()
    claims = []
    for line in lines:
        claims.append(convert_claim_to_loc(line))
    for claim in claims:
        write_claim(df_big, claim)
    for claim in claims:
        if is_claim_intact(df_big, claim):
            print("Claim is intact! #", claim[0])
            break
